 ...
 */
async function displayTasks(tasks, displaySettings = null) {
  ...
  for (const task of tasks) {
    ...
    taskElement
      ...
      .addEventListener("click", async (e) => {
        ...
        await popupLogger.info(
          `Botão 'Abrir' clicado para a tarefa: ${taskId}`
        );
    ...
    taskElement
      ...
      .addEventListener("click", async (e) => {
        ...
        await popupLogger.info(
          `Botão 'Ignorar' clicado para a tarefa: ${taskId}`
        );
        ...
        if (tasksList.children.length === 0) {
          ...
          tasksList.appendChild(noTasksP);
          await popupLogger.info(
            "Todas as tarefas removidas do popup após ignorar."
          );
        }
      });
 ...
 */
async function applyCustomSnooze(taskId) {
  ...
  if (!hoursInput || !minutesInput) {
    await popupLogger.error(
      `Inputs de tempo personalizado não encontrados para tarefa ${taskId}`
    );
  ...
  await popupLogger.info(
    `Aplicando snooze personalizado: ${hours}h ${minutes}min (${totalMinutes} minutos) para tarefa ${taskId}`
  );
  await applySnooze(taskId, totalMinutes);
}

/**
 * Aplica o snooze para uma tarefa
 */
async function applySnooze(taskId, minutes) {
  await popupLogger.info(
    `Aplicando snooze de ${minutes} minutos para a tarefa: ${taskId}`
  );
...
function closeSnoozeDropdownOnOutsideClick(event) {
  ...
  if (
    ...
    !event.target.closest('[data-action="snooze"]')
  ) {
    closeSnoozeDropdown();
  }
}
...
function setupMainEventListeners() {
  ...
  if (openOptionsButton) {
    openOptionsButton.addEventListener("click", async () => {
      await popupLogger.info(
        'Botão "Configurações" clicado. Abrindo página de opções.'
      );
      try {
        await browserAPI.runtime.openOptionsPage();
      } catch (error) {
        await popupLogger.error("Erro ao abrir página de opções:", error);
        ...
        try {
          ...
          await popupLogger.error(
            "Erro no fallback para abrir opções:",
            fallbackError
          );
        }
      }
  ...
  browserAPI.runtime.onMessage.addListener(
    async (request, sender, sendResponse) => {
      await popupLogger.debug("Mensagem recebida no popup:", request);
...
function setupHelpSystem() {
  ...
  if (helpButton) {
    ...
    tooltipSystem.addTooltip(helpButton, {
      ...
      content:
        "Clique para abrir o guia completo da extensão com tutoriais, configurações e solução de problemas.",
  ...
  if (configButton) {
    tooltipSystem.addTooltip(configButton, {
      ...
      content:
        "Configure suas credenciais do SAU, intervalos de verificação, opções de notificação e personalização da interface.",
      position: "top",
      trigger: "hover",
    });
  }
  ...
  if (refreshButton) {
    tooltipSystem.addTooltip(refreshButton, {
      ...
      content:
        "Força uma verificação imediata por novas tarefas no SAU, ignorando o intervalo configurado.",
      position: "top",
      trigger: "hover",
    });
  }
...
async function checkFirstTimeUser() {
  try {
    ...
    await popupLogger.error("Erro ao verificar status de primeiro uso:", error);
  }
}
 ...
 */
async function showFirstTimeHelp() {
  const helpElement = document.getElementById("firstTimeHelp");
  if (helpElement) {
    helpElement.style.display = "block";
    await popupLogger.info("Ajuda para novos usuários exibida");
  }
}
...
async function startQuickTour() {
  await popupLogger.info("Iniciando tour rápido do popup");

  // Esconde a ajuda contextual
  await dismissFirstTimeHelp();
  ...
  const tourSteps = [
    {
      ...
      title: "🚀 Monitor SAU",
      content:
        "Esta é a interface principal da extensão. Aqui você vê suas tarefas novas e pode gerenciá-las.",
      position: "bottom",
    },
    {
      element: "#helpButton",
      ...
      content:
        "Clique aqui sempre que precisar de ajuda. Abre um guia completo com todas as funcionalidades.",
      position: "bottom",
    },
    {
      ...
      content:
        "Aqui você vê o status atual: se está verificando tarefas, quando foi a última verificação, etc.",
      position: "bottom",
    },
    {
      element: "#tasks-list",
      ...
      content:
        "Suas tarefas novas aparecem aqui. Você pode abrir, ignorar, ver detalhes ou adiar cada tarefa.",
      position: "top",
    },
    {
      element: "#openOptions",
      title: "⚙️ Configurações",
      content:
        "IMPORTANTE: Configure suas credenciais do SAU aqui para a extensão funcionar.",
      position: "top",
    },
    {
      ...
      title: "🔄 Atualizar",
      content:
        "Use este botão para verificar tarefas imediatamente, sem esperar o intervalo automático.",
      position: "top",
    },
  ];
 ...
 */
async function runQuickTour(steps) {
  let currentStep = 0;
  ...
  async function showStep(stepIndex) {
    ...
    if (!element) {
      await popupLogger.warn(
        `Elemento não encontrado para o passo ${stepIndex}: ${step.element}`
      );
    ...
    const tourTooltip = {
      ...
      content:
        ...
        ${
          stepIndex > 0
            ? '<button onclick="tourPrev()" style="margin-right: 8px; padding: 4px 8px; border: none; border-radius: 4px; background: #ccc; cursor: pointer;">← Anterior</button>'
            : ""
        }
...
async function markTourCompleted() {
  try {
    ...
    await popupLogger.info("Tour rápido marcado como completado");
  } catch (error) {
    await popupLogger.error("Erro ao marcar tour como completado:", error);
  }
}
...
async function dismissFirstTimeHelp() {
  ...
  try {
    ...
    await popupLogger.info("Ajuda para novos usuários dispensada");
  } catch (error) {
    await popupLogger.error("Erro ao dispensar ajuda:", error);
  }
}
...
async function openHelpPage() {
  await popupLogger.info("Abrindo página de ajuda");
