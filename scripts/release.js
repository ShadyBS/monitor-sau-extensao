...
class ReleaseManager {
  ...
  async validateEnvironment() {
    ...
    try {
      ...
      throw new ReleaseError("Não está em um repositório Git", "NOT_GIT_REPO");
    }
  ...
  async extractReleaseNotes(version) {
    try {
      ...
      console.warn(
        "⚠️  Não foi possível extrair notas de release do CHANGELOG"
      );
      return `Release ${version}\n\nVeja CHANGELOG.md para detalhes das mudanças.`;
    }
  }
    ...
    try {
      ...
      if (releaseExists) {
        ...
        for (const zipFile of zipFiles) {
          try {
            execSync(
              `gh release upload v${version} "${zipFile.zipPath}" --clobber`,
              { stdio: "inherit" }
            );
          } catch (error) {
            console.warn(
              `⚠️  Falha ao fazer upload de ${zipFile.zipName}: ${error.message}`
            );
          }
        }
      } else {
        ...
        console.log(
          "   (ou execute: gh release upload v" + version + " <arquivo>)"
        );
      }
  ...
  async release(options = {}) {
    try {
      ...
      if (process.stdin.isTTY && !options.autoConfirm) {
        ...
        console.log(
          `   Arquivos: ${zipFiles.map((f) => f.zipName).join(", ")}`
        );
        console.log("\n📝 Notas de Release:");
        console.log(
          releaseNotes
      ...
      switch (error.code) {
        ...
        case "UNCOMMITTED_CHANGES":
          console.error(
            '   - Execute git add . && git commit -m "suas mudanças"'
          );
          break;
        ...
        case "CHANGELOG_NOT_UPDATED":
          console.error(
            "   - Atualize CHANGELOG.md com as mudanças desta versão"
          );
          break;
      }
